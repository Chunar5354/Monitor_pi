...
class Hunter():
	...
	def _handle(self, para, sql):
		...
		change_result = result.astype(str)  # change data type into string 
		num_result = np.array(change_result)  # change the whole type into list, by using numpy.array
		list_result = num_result.tolist()  # into list

		self.result_dict[para] = list_result
	...
	def get_data(self, data_dict):
		...
		for para in para_list:
			...
			th_list.append(threading.Thread(target=self._handle, args=(para, sql)))
